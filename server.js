// ==================== YOUR BRAND – FINAL SERVER.JS (VERCEL READY) ====================
...
// ---------- RATE LIMITER (NO CONFLICT WITH PROXY) ----------
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 200,
  standardHeaders: true,
  legacyHeaders: false,
  validate: false,                     // avoids proxy validation error
  keyGenerator: (req) => req.ip,       // uses IP already resolved by Vercel
});
app.use('/api', limiter);
...
async function sendEmail({ to, subject, html }) {
  if (!transporter) {
    console.log(`Email not configured. OTP for ${to}: ${html.replace(/<[^>]*>/g,'')}`);
    return;
  }
  try {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!email || !password || !fullName) return res.status(400).json({ err: 'All fields required' });
    if (password.length < 6) return res.status(400).json({ err: 'Password too short' });

    let user = await User.findOne({ email });
    if (user?.isVerified) return res.status(400).json({ err: 'Email already registered' });
    if (user && user.otpRequestedAt && (Date.now() - user.otpRequestedAt) < 60000) {
      return res.status(429).json({ err: 'Wait 60s before requesting a new OTP' });
    }
...
app.post('/api/auth/verify-otp', async (req, res) => {
  try {
    ...
    if (!user || !user.otp || user.otpExpire < Date.now()) {
      return res.status(400).json({ err: 'OTP expired or invalid' });
    }
    if (user.otp !== otp) {
...
app.post('/api/leads/product-inquiry', async (req, res) => {
  try {
    ...
    const lead = await Lead.create({
      ...
      guestName: req.user?.fullName || req.body.guestName || 'Guest',
      guestEmail: req.user?.email || req.body.guestEmail || '',
      guestPhone: req.user?.phone || req.body.guestPhone || '',
      customer: req.user?._id,
    });
    const waText = `🛍️ *NEW INQUIRY*\n📋 ${inquiryId}\n👤 ${lead.guestName}\n📦 ${lead.productName}\n💰 ${lead.estimatedBudget}`;
    const waLink = `https://wa.me/${process.env.WHATSAPP_NUMBER}?text=${encodeURIComponent(waText)}`;
    if (lead.guestEmail) {
      await sendEmail({ to: lead.guestEmail, subject: 'Inquiry Received', html: `<p>Thank you! Inquiry ID: ${inquiryId}</p>` }).catch(() => {});
    }
